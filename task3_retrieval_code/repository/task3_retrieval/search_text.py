...
def main():
    ...
    q_vec = embedder.encode_text(args.query).astype(np.float32)[None, :]
    ...
    q_img = Image.new("RGB", (224, 224), (255, 255, 255))
    out_path = os.path.join(args.out_dir, f"retrieval_text_k{args.k}.png")
    save_retrieval_grid(
        query_img=q_img,
        retrieved=[(im.convert("RGB").resize((224, 224)), lab, dist, ds_i) for (im, lab, dist, ds_i) in retrieved],
        out_path=out_path,
        title=f"Text query: {args.query}",
    )
